...
function contextMenuShow(event) {
  ...
  $.ajax({
    ...
    success: function(data, textStatus, jqXHR) {
      ...
      // find submenu offset position
      var submenuPosition = $('#context-menu .folder').offset();

      // calculate how much available space we have under and above the submenu
      var availableSpaceBelow = window_height - (submenuPosition.top + $('#context-menu .folder').height());
      var availableSpaceAbove = submenuPosition.top;

      // adjust the position of the submenu
      if (availableSpaceBelow < 0 && availableSpaceAbove > availableSpaceBelow) {
        $('#context-menu .folder').removeClass('down').addClass('up');
      } else {
        $('#context-menu .folder').removeClass('up').addClass('down');
      }
